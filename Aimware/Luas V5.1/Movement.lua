...
local VelocityIndicatorCheckbox = gui.Checkbox(IndicatorsGroupBox, "MovementIndicatorsVelocityIndicator", "Velocity Indicator", false);
local VelocityIndicatorTakeOffCheckbox = gui.Checkbox(IndicatorsGroupBox, "MovementIndicatorsVelocityIndicatorTakeOff", "Show Take OFF", false);
local VelocityIndicatorAlphaCheckbox = gui.Checkbox(IndicatorsGroupBox, "MovementIndicatorsVelocityIndicatorAlpha", "Velocity Alpha", false);
local StaminaIndicatorCheckbox = gui.Checkbox(IndicatorsGroupBox, "MovementIndicatorsStamina", "Stamina Inicator", false);
local StaminaStaticColorPicker = gui.ColorPicker(IndicatorsGroupBox, "MovementIndicatorsStaminaStaticColorPicker", "Stamina Staic Color", 255, 255, 255, 255);
local IndicatorsFeaturesMultiBox = gui.Multibox(IndicatorsGroupBox, "Indicator Options");

local IndicatorsJumpBug = gui.Checkbox(IndicatorsFeaturesMultiBox, "MovementIndicatorsIndicatorsFeaturesMultiboxJumpBug", "Jump Bug", false);
local IndicatorsLongJump = gui.Checkbox(IndicatorsFeaturesMultiBox, "MovementIndicatorsIndicatorsFeaturesMultiboxLongJump", "Long Jump", false);
local IndicatorsMiniJump = gui.Checkbox(IndicatorsFeaturesMultiBox, "MovementIndicatorsIndicatorsFeaturesMultiboxMiniJump", "Mini Jump", false);
local IndicatorsEdgeBug = gui.Checkbox(IndicatorsFeaturesMultiBox, "MovementIndicatorsIndicatorsFeaturesMultiboxEdgeBug", "EdgeBug", false);
local IndicatorsColor = gui.ColorPicker(IndicatorsFeaturesMultiBox, "MovementIndicatorsIndicatorsFeaturesMultiboxIndicatorsColor", "Indicator Color", 255, 255, 255, 255);

local IndicatorPosSlider = gui.Slider(IndicatorsGroupBox, "MovementIndicatorsIndicatorPos", "Indicator Postion", 80, 0, 100);
...
function DrawIndicator()
    if (entities.GetLocalPlayer() and entities.GetLocalPlayer():IsAlive()) then
        ...
        local vX, vY = LocalPlayer:GetPropFloat("localdata", 'm_vecVelocity[0]'), LocalPlayer:GetPropFloat("localdata", 'm_vecVelocity[1]');
        ...
        draw.Text(MidX - (IndicatorTextWidht / 2) + 1, MidY - (IndicatorTextHeight / 2) + PositionAbsolute - 31 + 1, IndicatorsFeaturesCompostion);
        draw.Color(IndicatorColorR, IndicatorColorG, IndicatorColorB, IndicatorColorA);
        draw.Text(MidX - (IndicatorTextWidht / 2), MidY - (IndicatorTextHeight / 2) + PositionAbsolute - 31, IndicatorsFeaturesCompostion);
