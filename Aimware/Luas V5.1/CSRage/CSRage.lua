localVersion = "0.1-0002"
githubVersion = http.Get("https://raw.githubusercontent.com/MentalIssue/Cheat-Configs/tree/main/Aimware/CSRage/version.txt")

if githubVersion then
    if githubVersion ~= localVersion then
        print("Updating Lua...")
        file.Delete(GetScriptName())
        file.Write(GetScriptName(), http.Get("https://raw.githubusercontent.com/MentalIssue/Cheat-Configs/tree/main/Aimware/CSRage/CSRage.lua"))
...
local Gui = {
    ...
    Init = function(self)
        ...
        local rageTab = gui.Tab(rageRef, "csrage.tab", "CSRage")
        local aaRef = gui.Groupbox(rageTab, "Anti-Aim", 15, 15, 297, 300)
        local visualsRef = gui.Groupbox(rageTab, "Visuals", 327, 15, 297, 300)

        self.aaMode = gui.Combobox(aaRef, "csrage.mode", "Mode", "Recommended", "Custom")

        self.customAAItems.yawSlider = gui.Slider(aaRef, "csrage.yaw", "Yaw", 180, -180, 180)
        self.customAAItems.flickYawSlider = gui.Slider(aaRef, "csrage.flickyaw", "Flick Yaw", 160, -180, 180)

        self.customAAItems.desyncSlider = gui.Slider(aaRef, "csrage.desync", "Desync", -58, -58, 58)
        self.customAAItems.flickDesyncSlider = gui.Slider(aaRef, "csrage.flickdesync", "Flick desync", -58, -58, 58)

        self.customAAItems.modeCombobox = gui.Combobox(aaRef, "csrage.desyncmode", "Desync Mode", "Desync", "Desync Jitter")
        self.customAAItems.flickModeCombobox = gui.Combobox(aaRef, "csrage.flickdesyncmode", "Flick Desync Mode", "Desync", "Desync Jitter")

        self.customAAItems.pitchBox = gui.Combobox(aaRef, "csrage.pitch", "Pitch", "Off", "89", "0", "180 (Untrusted)")
        self.customAAItems.flickPitchBox = gui.Combobox(aaRef, "csrage.flickpitch", "Flick Pitch", "Off", "89", "0", "180 (Untrusted)")

        self.customAAItems.flickSlider = gui.Slider(aaRef, "csrage.whentoflick", "When to flick", 10, 0, 100)

        self.customAAItems.invertFlickCheckbox = gui.Checkbox(aaRef, "csrage.invertonflick", "Invert Flick", true)
        self.customAAItems.invertFlickCheckbox:SetDescription("Invert Flick everytime it flicks")

        self.visualItems.indicatorsCheckbox = gui.Checkbox(visualsRef, "csrage.indicators", "Indicators", false)
        self.visualItems.hitmarkerMultiCombobox = gui.Multibox(visualsRef, "Hitmarker")

        self.visualItems.hitmarkerScreenCheckbox = gui.Checkbox(self.visualItems.hitmarkerMultiCombobox, "csrage.screenhitmarker", "Screen Hitmarker", false)
        self.visualItems.hitmarkerScreenColorpicker = gui.ColorPicker(self.visualItems.hitmarkerScreenCheckbox, "csrage.screenhitmarkercolor", "", 255, 255, 255, 255)
        self.visualItems.hitmarkerScreenHSColorpicker = gui.ColorPicker(self.visualItems.hitmarkerScreenCheckbox, "csrage.screenhitmarkerhscolor", "", 220, 60, 40, 255)

        self.visualItems.hitmarkerWorldCheckbox = gui.Checkbox(self.visualItems.hitmarkerMultiCombobox, "csrage.worldhitmarker", "World Hitmarker", false)
        self.visualItems.hitmarkerWorldColorpicker = gui.ColorPicker(self.visualItems.hitmarkerWorldCheckbox, "csrage.worldhitmarkercolor", "", 255, 255, 255, 255)
        self.visualItems.hitmarkerWorldHSColorpicker = gui.ColorPicker(self.visualItems.hitmarkerWorldCheckbox, "csrage.worldhitmarkerhscolor", "", 220, 60, 40, 255)
    end;
...
local Visuals = {
    ...
    sizes = { --we dont wanna rape fps :)
        ...
        rageSize = draw.GetTextSize("Rage"),
    ...
    Indicators = function(self)
        ...
        local rageWidth, rageHeight = self.sizes.rageSize
        ...
        draw.TextShadow(screenWidth / 2 - math.floor(rageWidth / 2) - math.floor(luaWidth / 2) + self.scopeOffset, screenHeight / 2 + offset, "rage")

        draw.Color(220, 60, 40, 255)
        draw.TextShadow(screenWidth / 2 + math.floor(rageWidth / 2) - math.floor(luaWidth / 2) - 1 + self.scopeOffset, screenHeight / 2 + offset, "Lua")
        offset = offset + 15
